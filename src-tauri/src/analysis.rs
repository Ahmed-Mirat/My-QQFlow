...
fn build_type_distribution(counts: &HashMap<String, i64>) -> Vec<TypeDistItem> {
    let types = [
        "text", "image", "voice", "video", "system", "miniapp", "other",
    ];
    let names = ["文本", "图片", "语音", "视频", "系统", "小程序", "其他"];
    types
        .iter()
        .zip(names.iter())
        .map(|(k, n)| TypeDistItem {
            name: n.to_string(),
            value: *counts.get(*k).unwrap_or(&0),
        })
        .collect()
}
...
pub fn list_groups(
    ...
    if let Some(store) = store {
        let groups: Vec<GroupInfo> = store
            .group_msgs
            .iter()
            .map(|(id, msgs)| GroupInfo {
                id: id.clone(),
    ...
    let groups: Vec<GroupInfo> = stmt
        ...
        .flatten()
        .map(|mut g| {
            g.name = group_names.get(&g.id).cloned().unwrap_or_default();
            g
        })
        .collect();
...
fn build_group_result(
    ...
    member_first: &HashMap<String, String>,
    member_last: &HashMap<String, String>,
    hourly: &HashMap<i32, i64>,
    weekday_counts: &HashMap<i32, i64>,
    ...
    let mut ranking: Vec<(String, i64)> =
        member_counts.iter().map(|(k, v)| (k.clone(), *v)).collect();
    ranking.sort_by(|a, b| b.1.cmp(&a.1));
    let member_ranking: Vec<MemberStat> = ranking
        .iter()
        .take(50)
        .map(|(name, count)| MemberStat {
            name: name.clone(),
            message_count: *count,
        ...
        .collect();
    let hourly_dist: HashMap<String, i64> = (0..24)
        .map(|h| (h.to_string(), *hourly.get(&h).unwrap_or(&0)))
        .collect();
    let weekday_dist: HashMap<String, i64> = (0..7)
        .map(|w| (weekday_name(w), *weekday_counts.get(&w).unwrap_or(&0)))
        .collect();
    ...
    let top_phrases: Vec<PhraseStat> = pv
        .into_iter()
        .filter(|(_, c)| *c >= 2)
        .take(20)
        .map(|(phrase, count)| PhraseStat { phrase, count })
        .collect();
    Ok(GroupAnalysisData {
        group_id: group_id.to_string(),
        total_messages: total,
        member_count: member_ranking.len(),
        member_ranking,
        ...
        top_phrases,
        first_message: ts_to_str(first_id),
        last_message: ts_to_str(last_id),
    })
}
...
pub fn analyze_private_detail(
    ...
    if let Some(store) = store {
        ...
        if total == 0 {
            return Err("未找到聊天数据".to_string());
        }
        return build_private_result(
            total,
            daily.len(),
            &counts,
            &first_ts,
...
pub fn list_contacts(
        ...
        let groups: Vec<GroupInfo> = store
            ...
            .map(|(id, msgs)| {
                let name = store
                    ...
                    .get(id)
                    .cloned()
                    .or_else(|| store.uid_map.get(id).cloned())
    ...
    let conversation_col = super::export_chat::c2c_conversation_column(&conn);
    let sql = format!(
        "SELECT \"{0}\", count(*) FROM c2c_msg_table GROUP BY \"{0}\"",
        conversation_col,
    );
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
    let groups: Vec<GroupInfo> = stmt
        ...
        .flatten()
        .map(|mut g| {
            g.name = profile_names
                .get(&g.id)
                .cloned()
                .or_else(|| uid_map.get(&g.id).cloned())
                .unwrap_or_default();
            g
        })
        .collect();
...
fn build_private_result(
    ...
    contact_first: &HashMap<String, String>,
    contact_last: &HashMap<String, String>,
    hourly: &HashMap<i32, i64>,
    weekday_counts: &HashMap<i32, i64>,
    monthly: &HashMap<String, i64>,
    type_counts: &HashMap<String, i64>,
    ...
    let mut ranking: Vec<(String, i64)> = contact_counts
        .iter()
        .map(|(k, v)| (k.clone(), *v))
        .collect();
    ranking.sort_by(|a, b| b.1.cmp(&a.1));
    let contact_ranking: Vec<MemberStat> = ranking
        .iter()
        .take(50)
        .map(|(name, count)| MemberStat {
            name: name.clone(),
    ...
    Ok(PrivateAnalysisData {
        total_messages: total,
        active_days,
        contact_count: contact_ranking.len(),
        contact_ranking,
        hourly_distribution: hourly_dist,
        weekday_distribution: weekday_dist,
        monthly_distribution: monthly.iter().map(|(k, v)| (k.clone(), *v)).collect(),
...
pub fn export_csv(
    ...
    if let Some(store) = store {
        ...
        for gid in &group_ids {
            ...
            wtr.write_record(["时间", "发送者", "类型", "内容"])
                .map_err(|e| e.to_string())?;
            for m in msgs {
                let sender = if !m.nick.is_empty() {
                    &m.nick
                } else {
                    uid_map.get(&m.uid).map(|s| s.as_str()).unwrap_or(&m.uid)
                };
                let parsed = extract_text(&m.blob);
                wtr.write_record([
                    &ts_to_str(m.msg_id),
            ...
            if !msgs.is_empty() {
                total += msgs.len();
                group_count += 1;
                files.push(csv_path.to_string_lossy().to_string());
            }
        }

        let private_dir = std::path::Path::new(output_dir).join("私聊表格");
        fs::create_dir_all(&private_dir).ok();
        let peer_filter: Option<std::collections::HashSet<&str>> =
        ...
        for (peer, msgs) in &store.c2c_msgs {
            ...
            let label =
                super::export_chat::c2c_contact_label(peer, msgs, uid_map, &store.profile_names);
            let csv_path = private_dir.join(format!(
                "{}.csv",
                crate::export_chat::sanitize_filename(&label)
            ));
            let mut file = std::fs::File::create(&csv_path).map_err(|e| e.to_string())?;
            use std::io::Write;
            file.write_all(&[0xEF, 0xBB, 0xBF])
    ...
    let conn = super::export_chat::open_db_for_analysis(db_path, key)?;
    let uid_map = super::export_chat::load_uid_map(&conn);
    let c2c_conversation_col = super::export_chat::c2c_conversation_column(&conn);
    let mut total = 0usize;
    let mut group_count = 0usize;
    let mut private_count = 0usize;
    let mut files = Vec::new();
    ...
    for gid in &group_ids {
        ...
        for row in rows.flatten() {
            ...
            let sender = if !nick.is_empty() {
                ...
                uid_map.get(uid.as_str()).cloned().unwrap_or(uid)
            };
            let parsed = extract_text(&blob);
            wtr.write_record([
                &ts_to_str(msg_id),
